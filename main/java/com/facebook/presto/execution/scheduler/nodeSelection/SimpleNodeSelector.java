        ...
        implements NodeSelector
{
    ...
    public List<InternalNode> getActiveNodes()
    {
        ...
        return selectNodes(stageID.get_possibleNodesId(), nodeMap.get().get().getActiveNodes());
    }
    ...
    public List<InternalNode> getAllNodes()
    {
        ...
        return selectNodes(stageID.get_possibleNodesId(), nodeMap.get().get().getAllNodes());
    }
    ...
    public List<InternalNode> selectRandomNodes(int limit, Set<InternalNode> excludedNodes)
    {
        ...
        List<String> possibleNodes = stageID.get_possibleNodesId();
        return selectNodes(possibleNodes, randomizedNodes(nodeMap.get().get(), includeCoordinator, excludedNodes));
    }
    ...
    public SplitPlacementResult computeAssignments(Set<Split> splits, List<RemoteTask> existingTasks)
    {
        ...
        List<String> possibleNodes = stageID.get_possibleNodesId();
        List<InternalNode> eligibleNodes = getEligibleNodes_from_file(possibleNodes, nodeMap, existingTasks);
        ...
        for (Split split : splits) {
            List<InternalNode> candidateNodes = ImmutableList.of();
            switch (split.getNodeSelectionStrategy()) {
                case HARD_AFFINITY:
                    ...
                    log.debug("visit Hard_affinity");
                    candidateNodes = candidateNodes.stream().filter(eligibleNodes::contains).collect(toList());
    ...
    {
        LOG.debug("NotVisiting getEligibleNodes");
    ...
    private List<InternalNode> getEligibleNodes_from_file(List<String> possibleNodes, NodeMap nodeMap, List<RemoteTask> existingTasks)
    {
        LOG.debug("Visiting getEligibleNodes_from_file");
        List<InternalNode> existingNodes = existingTasks.stream()
                .map(remoteTask -> nodeMap.getActiveNodesByNodeId().get(remoteTask.getNodeId()))
                // nodes may sporadically disappear from the nodeMap if the announcement is delayed
                .filter(Objects::nonNull)
                .collect(toList());
        List<InternalNode> moreNodes = selectNodes(possibleNodes, randomizedNodes(nodeMap, includeCoordinator, newHashSet(existingNodes)));
        existingNodes.addAll(moreNodes);
        verify(existingNodes.stream().allMatch(Objects::nonNull), "existingNodes list must not contain any nulls");
        return existingNodes;
    }
