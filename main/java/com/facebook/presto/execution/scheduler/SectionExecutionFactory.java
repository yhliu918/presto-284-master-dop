...
public class SectionExecutionFactory {
    ...
    public SectionExecution createSectionExecutions(
            ...
            int attemptId) {
        // Only fetch a distribution once per section to ensure all stages see the same
        // machine assignments
        Map<Pair<PartitioningHandle, StageId>, NodePartitionMap> partitioningCache = new HashMap<>();
        ...
        List<StageExecutionAndScheduler> sectionStages = createStreamingLinkedStageExecutions(
                ...
                PartitioningHandle -> StageId -> partitioningCache.computeIfAbsent(
                        new Pair(PartitioningHandle, StageId),
                        key -> nodePartitioningManager.getNodePartitioningMap(session, key.getFirst(), nodePredicate,
                        Optional.of(key.getSecond()))),
                tableWriteInfo,
                Optional.empty(),
                summarizeTaskInfo,
                remoteTaskFactory,
    ...
    /**
     * returns a List of StageExecutionAndSchedulers in a postorder representation
     * of the tree
     */
    private List<StageExecutionAndScheduler> createStreamingLinkedStageExecutions(
            ...
            Function<PartitioningHandle, Function<StageId, NodePartitionMap>> partitioningCache,
        ...
        // Optional<int[]> bucketToPartition = getBucketToPartition(partitioningHandle,
        // partitioningCache, plan.getFragment().getRoot(), remoteSourceNodes);
        Optional<int[]> bucketToPartition = getBucketToPartition(partitioningHandle, partitioningCache,
                plan.getFragment().getRoot(), remoteSourceNodes, stageId);
        for (int i = 0; i < bucketToPartition.get().length; i++) {
            log.debug("bucketToPartition: %d, %d", i, bucketToPartition.get()[i]);
        }
    ...
    private StageScheduler createStageScheduler(
        ...
        if (partitioningHandle.equals(SOURCE_DISTRIBUTION)) {
            // nodes are selected dynamically based on the constraints of the splits and the
            // system load
            ...
            NodeSelector nodeSelector = nodeScheduler.createNodeSelector(session, connectorId, maxTasksPerStage,
                    nodePredicate, Optional.of(stageId));
            SplitPlacementPolicy placementPolicy = new DynamicSplitPlacementPolicy(nodeSelector,
                    stageExecution::getAllTasks);

            checkArgument(!plan.getFragment().getStageExecutionDescriptor().isStageGroupedExecution());
            log.debug("Attention SOURCE_DISTRIBUTION");
            return newSourcePartitionedSchedulerAsStageScheduler(stageExecution, planNodeId, splitSource,
                    placementPolicy, splitBatchSize);
        } else if (partitioningHandle.equals(SCALED_WRITER_DISTRIBUTION)) {
            log.debug("Attention SCALED_WRITER_DISTRIBUTION");
            ...
            ScaledWriterScheduler scheduler = new ScaledWriterScheduler(
                    ...
                    sourceTasksProvider,
                    writerTasksProvider,
                    nodeScheduler.createNodeSelector(session, null, nodePredicate, Optional.of(stageId)),
                    scheduledExecutor,
            ...
            if (!splitSources.isEmpty()) {
                ...
                boolean groupedExecutionForStage = plan.getFragment().getStageExecutionDescriptor()
                        .isStageGroupedExecution();
                if (groupedExecutionForStage) {
                    connectorPartitionHandles = nodePartitioningManager.listPartitionHandles(session,
                            partitioningHandle);
                ...
                if (plan.getFragment().getRemoteSourceNodes().stream()
                        .allMatch(node -> node.getExchangeType() == REPLICATE)) {
                    // no non-replicated remote source
                    boolean dynamicLifespanSchedule = plan.getFragment().getStageExecutionDescriptor()
                            .isDynamicLifespanSchedule();
                    bucketNodeMap = nodePartitioningManager.getBucketNodeMap(session, partitioningHandle,
                            dynamicLifespanSchedule);

                    // verify execution is consistent with planner's decision on dynamic lifespan
                    // schedule
                    ...
                    if (bucketNodeMap.hasInitialMap()) {
                        ...
                        log.debug("hasInitialMap");
                    } else {
                        // stageNodeList = new ArrayList<>(nodeScheduler.createNodeSelector(session,
                        // connectorId, nodePredicate).selectRandomNodes(maxTasksPerStage));
                        stageNodeList = new ArrayList<>(nodeScheduler.createNodeSelector(session, connectorId, Optional.empty(), Optional.of(stageId))
                                .selectRandomNodes(maxTasksPerStage));
                    }
                } else {
                ...
                FixedSourcePartitionedScheduler fixedSourcePartitionedScheduler = new FixedSourcePartitionedScheduler(
                        ...
                        nodeScheduler.createNodeSelector(session, connectorId, nodePredicate, Optional.of(stageId)),
                ...
                if (plan.getFragment().getStageExecutionDescriptor().isRecoverableGroupedExecution()) {
                    stageExecution.registerStageTaskRecoveryCallback(taskId -> {
                        checkArgument(taskId.getStageExecutionId().getStageId().equals(stageId),
                                "The task did not execute this stage");
                        checkArgument(parentStageExecution.isPresent(), "Parent stage execution must exist");
                        checkArgument(parentStageExecution.get().getAllTasks().size() == 1,
                                "Parent stage should only have one task for recoverable grouped execution");
    ...
    private static Optional<int[]> getBucketToPartition(
        ...
        } else if (PlanNodeSearcher.searchFrom(fragmentRoot).where(node -> node instanceof TableScanNode).findFirst()
                .isPresent()) {
            if (remoteSourceNodes.stream().allMatch(node -> node.getExchangeType() == REPLICATE)) {
                return Optional.empty();
            } else {
                // remote source requires nodePartitionMap
                NodePartitionMap nodePartitionMap = partitioningCache.apply(partitioningHandle).apply(stageId);
                log.debug("getBucketToPartition 1");
                return Optional.of(nodePartitionMap.getBucketToPartition());
            }
        } else {
            NodePartitionMap nodePartitionMap = partitioningCache.apply(partitioningHandle).apply(stageId);
            log.debug("getBucketToPartition 2");
            List<InternalNode> partitionToNode = nodePartitionMap.getPartitionToNode();
            // todo this should asynchronously wait a standard timeout period before failing
            checkCondition(!partitionToNode.isEmpty(), NO_NODES_AVAILABLE, "No worker nodes available");
            return Optional.of(nodePartitionMap.getBucketToPartition());
    ...
    private static ListenableFuture<?> whenAllStages(Collection<SqlStageExecution> stageExecutions,
            Predicate<StageExecutionState> predicate) {
