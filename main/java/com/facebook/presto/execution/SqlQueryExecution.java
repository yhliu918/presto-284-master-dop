        ...
        implements QueryExecution
{
    ...
    private SqlQueryExecution(
            ...
            Optional<PlanPrinter> planPrinter)
    {
        try (SetThreadName ignored = new SetThreadName("Query-%s", stateMachine.getQueryId())) {
            ...
            this.planPrinter = planPrinter.orElse(null);
    ...
    public void start()
    {
        try (SetThreadName ignored = new SetThreadName("Query-%s", stateMachine.getQueryId())) {
            try {
                ...
                String distPlan = PlanPrinter.jsonDistributedPlan(plan.getRoot(), metadata.getFunctionAndTypeManager());
                log.debug("the plan is printed to be %s", distPlan);
    ...
    private PlanRoot createLogicalPlanAndOptimize()
    {
        try {
            ...
            Deque<PlanNode> stackOrigin = new ArrayDeque<>();
            Deque<String> prefixOrigin = new ArrayDeque<>();
            stackOrigin.push(planNode);
            prefixOrigin.push("");

            while (!stackOrigin.isEmpty()) {
                PlanNode currentNode = stackOrigin.pop();
                String currentPrefix = prefixOrigin.pop();
                System.out.println(currentPrefix + currentNode.getId());

                // Push child nodes onto the stack in reverse order (for DFS)
                List<PlanNode> children = currentNode.getSources();
                if (children.size() == 1) {
                    stackOrigin.push(children.get(0));
                    prefixOrigin.push(currentPrefix);
                }
                else {
                    for (int i = children.size() - 1; i >= 0; i--) {
                        stackOrigin.push(children.get(i));
                        prefixOrigin.push(currentPrefix + "-");
                    }
                }
            }
            ...
            Map<PlanNodeId, PlanNode> planNodeMap = plan.getPlanIdNodeMap(); 
            for (Map.Entry<PlanNodeId, PlanNode> entry : planNodeMap.entrySet()) {
                System.out.println("PlanNodeId: " + entry.getKey().getId());
            }

            PlanNode rootNode = plan.getRoot();
            ...
            while (!stack.isEmpty()) {
                PlanNode currentNode = stack.pop();
                String currentPrefix = prefix.pop();
                System.out.println(currentPrefix + currentNode.getId());
